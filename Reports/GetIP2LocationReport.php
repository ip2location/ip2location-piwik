...
namespace Piwik\Plugins\IP2Location\Reports;
 ...
 */
class GetIP2LocationReport extends Base
{
    protected function init()
    {
        parent::init();

        $this->name          = Piwik::translate('IP2Location_IP2LocationReport');
        $this->dimension     = null;
        ...
        // $this->menuTitle    = 'IP2Location_IP2LocationReport';
        ...
        // $this->widgetTitle  = 'IP2Location_IP2LocationReport';

		$this->menuTitle = 'IP2Location Report';
		$this->metrics = array('date', 'ip_address', 'ip_country', 'ip_region', 'ip_city', 'ip_latitude', 'ip_longitude', 'ip_zip_code', 'ip_time_zone', 'ip_usage_type');
    }
    ...
    public function configureView(ViewDataTable $view)
    {
        if (!empty($this->dimension)) {
            $view->config->addTranslations(array('date' => $this->dimension->getName()));
        }
		...
		$view->config->addTranslation('date', 'Date');
		$view->config->addTranslation('ip_address', 'IP Address');
		$view->config->addTranslation('ip_country', 'Country');
		$view->config->addTranslation('ip_region', 'Region Name');
		$view->config->addTranslation('ip_city', 'City Name');
		$view->config->addTranslation('ip_latitude', 'Latitude');
		$view->config->addTranslation('ip_longitude', 'Longitude');
		$view->config->addTranslation('ip_zip_code', 'ZIP Code');
		$view->config->addTranslation('ip_time_zone', 'Time Zone');
		$view->config->addTranslation('ip_usage_type', 'Usage Type');
    ...
    {
        $view = new View('@IP2Location/getIP2LocationReport');
        $view->myData = array();
